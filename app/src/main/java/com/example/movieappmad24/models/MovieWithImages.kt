...
fun getMoviesWithImages(): List<MovieWithImages> {
    ...
    for (movieHelper in movieHelpers) {
        val movie = Movie(
            id = movieHelper.id,
            title = movieHelper.title,
            year = movieHelper.year,
            genre = movieHelper.genre,
            director = movieHelper.director,
            actors = movieHelper.actors,
            plot = movieHelper.plot,
            trailer = movieHelper.trailer,
            rating = movieHelper.rating,
            isFavorite = movieHelper.isFavorite
        )
        val movieWithImages = MovieWithImages(movie, movieHelper.images)
        movieWithImagesList.add(movieWithImages)
    }

    return movieWithImagesList
}

fun getMovies(): List<MovieHelper> {
    return listOf(
        MovieHelper(id = "tt0499549",
            title = "Avatar",
            year = "2009",
